...
import top.wxip.carlink.server.wrapper.ActivityManager;
...
public final class AudioCapture {
    ...
    private static void startWorkaroundAndroid11() {
        ...
        ActivityManager.getInstance().startActivityAsUserWithFeature(intent);
    }

    private static void stopWorkaroundAndroid11() {
        ActivityManager.getInstance().forceStopPackage(FakeContext.PACKAGE_NAME);
    }
    ...
    public int read(ByteBuffer directBuffer, int size, MediaCodec.BufferInfo outBufferInfo) {
        ...
        if (ret == AudioRecord.SUCCESS) {
            ...
            if (nextPts == 0) {
                Ln.e("Could not get any audio timestamp");
            }
